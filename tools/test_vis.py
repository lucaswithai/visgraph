...
class StaticImageDemo:
    ...
    def __call__(self, image,pre_image):
        predictions = self.predictor(image,pre_image)
        return predictions
...
class StaticImagePredictor:
    ...
    def __call__(self, input_image,input_image_pre):
        image = copy.deepcopy(input_image)
        pre_image=copy.deepcopy(input_image_pre)
        with torch.no_grad():
            ...
            input = {'image': image, 'height': height, 'width': width,'pre_image':pre_image}
...
if __name__ == '__main__':
    ...
    for video in tqdm(data['videos']):
        memory = dict()
        images = [
            read_image(osp.join('datasets/youtubevis/val/JPEGImages', image_name),
        ...
        for index in range(len(images)):
            if (index==0):
                ...
                pre_index=index-1
            predictions = demo(images[index],images[pre_index])[0]['instances']
